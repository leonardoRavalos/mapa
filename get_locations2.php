...
session_start();
$usuario_id = $_SESSION['user_id']; // ID del usuario que ha iniciado sesión

// Consulta para obtener ubicaciones de la célula del usuario que ha iniciado sesión
$query_locations = "SELECT c.id AS usuario_id, c.lat, c.lng, u.nombre
                     FROM coordenadas c
                     JOIN usuarios u ON c.usuario_id = u.id
                     WHERE u.tipo = 'celula' AND u.id = ?";
                     
$stmt = $conn->prepare($query_locations);
